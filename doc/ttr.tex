...
reflexive congruence generated by the following rules.
...
\end{theorem}
\begin{proof}
  The pre relations are structural over the proof terms. (Except for
  the record checking rule; but then the record types gets smaller in
  the local induction.)

  FIXME: the equality check is problematic to due to beta
  reduction. This is in fact a quite hairy issue.
\end{proof}
...
\begin{definition}[Elaboration of proofs]
  The proofs are translated as follows:
  \begin{align*}
    \trp{x} & = x \\
    \trp{t u} & = \trp t \trn u \\
    \trp{t.l} & = \trn t.l \\
    \trp{u} & = (\trn u : T)  & \text {otherwise} \\
    \trn{λx.t} & = (λx. \trn t) \\
