...
$\eval A {[x_i↦X_i, ∀ x_i:C_i ∈ Γ]}$ (see evaluation below).
...
\begin{mathpar}
  ...
  \inferrule
  {Γ ⊢ t \chk V  \\ Γ ⊢ (l_i = t_i) \chk [\app W {\eval t Γ} ] \\ l=t ∈ {l_i = t_i} }
  {Γ ⊢ (l_i = t_i) \chk [l : V; W]  }
...
\begin{theorem}
  If $Γ ⊢ t \chk V$, then $\eval t ρ$ terminates. [FIXME: this isn't
  true for records. Because of subtyping we will not check that
  certain fields are well typed, and those can thus
  diverge. Fortunately, such fields cannot be accessed (thanks to
  typing). Thus, if the evaluation is lazy, it will terminate in all
  allowed cases.]
\end{theorem}
\begin{proof}
  (idea) The case which could go wrong (application) is saved by types
  becoming smaller.
\end{proof}
\begin{theorem}
...
\begin{align*}
  (Π V W) ∧ (Π V' W') &= Π (V ∨ V') (ΛX. \app W X ∧ \app {W'} X) \\
  [] ∧ V       &= V \\
  ⊥ ∧ V       &= ⊥ \\
