...
\begin{mathpar}
  ...
  In the \ru{sub-refl} rule we write $\eval A Γ$ to mean $\eval A [x_i↦X_i, ∀ x_i:C_i ∈ Γ]$.
...
\begin{mathpar}
 ...
 \inferrule
  {Γ ⊢ t_i \chk A_i[l_i]  }
  {Γ ⊢ \splt {`l_i ↦ t_i, …} \chk (x : \fin { `l_i }) → A_i[x] }
...
\begin{mathpar}
 ...
 \inferrule
  {Γ ⊢ t_i \ifr A_i  }
  {Γ ⊢ \splt {`l_i ↦ t_i, …} \ifr (x : \fin { `l_i }) → ⋀_i A_i[x/`l, …_i] }
\end{mathpar}
